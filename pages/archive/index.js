        ...
        >
            <div className='flex items-center gap-4'>
                ...
                <div className="flex-1 truncate">
                    ...
                    <div className="flex items-start justify-end gap-1">
                        <p className='flex-1 text-sm truncate'>{user.email}</p>
                        {
                            user.isArchive &&
                            <Icon icon="material-symbols-light:archive-outline" className="text-lg shrink-0 text-red-500" />
                        }
                    </div>
                </div>
            </div>
                ...
                <div className="relative flex-1 grid grid-cols-12 lg:gap-10 overflow-hidden">
                    ...
                    <div className="col-span-12 xl:col-span-4 pt-2 overflow-hidden">
                        <div className="w-full h-full flex flex-col gap-3 overflow-auto">
                            ...
                            <div className="flex-1 border border-gray-200 overflow-auto">
                                {/* recent chat cards */}
                                <div className="flex flex-col-reverse justify-end divide-y divide-y-reverse divide-gray-200 pb-10">
                                    {/* {
                    ...
                    {/* chat box */}
                    <div className={`${selectedUser ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} absolute lg:relative z-10 w-full h-full lg:h-auto col-span-12 xl:col-span-8 overflow-hidden transition-all ease-in-out duration-500`}>
                        {
                            selectedUser ? (
                                <div className="w-full h-full flex flex-col bg-colorThemeSecondary border border-gray-200 divide-y divide-gray-200 overflow-auto">
                                    {/* head */}
                                    ...
                                    {/* body */}
                                    <div ref={chatContainerRef} className="flex-1 space-y-14 overflow-auto p-4 pb-10 customScrollBar">
                                        {
                                            messages.length > 0 && messages.map((message, index) => (
                                                <div key={index} className={`${message.sender === me ? 'flex-row-reverse' : ''} flex items-start gap-2`}>
                                                    ...
                                                    <div className={`${message.sender === me ? 'bg-slate-50' : 'bg-slate-50'} relative min-h-[38px] shadow flex flex-col items-start rounded-2xl px-4 py-2 gap-2`}>
                                                        ...
                                                        {
                                                            message.sender === me ? (
                                                                <span className="whitespace-pre-wrap font-medium">{myDummyData.firstName + ' ' + myDummyData.lastName}</span>
                                                            ) : (
                                                                <span className="whitespace-pre-wrap font-medium">{dummyUsersArr.find(e => e.id === message.sender).firstName + ' ' + dummyUsersArr.find(e => e.id === message.sender).lastName}</span>
                                                            )
                                                        }
                                                        {
                                                            message.message &&
                                                            <span className="whitespace-pre-wrap text-gray-600 text-sm">{message.message}</span>
                                                        }
                                                        {
                                                            message.updatedAt &&
